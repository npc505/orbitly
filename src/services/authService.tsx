  ...
  async login(username: string, password: string): Promise<LoginResponse> {
    try {
      ...
      if (response.status === 401) {
        throw { message: 'Usuario o contraseña incorrecto', status: 401 } as AuthError;
      }
    ...
    try {
        ...
        if (response.status === 400) {
        throw { message: 'Verifica que todos los datos sean correctos', status: 400 } as AuthError;
        }
        ...
        throw { message: 'Error de conexión.', status: 0 } as AuthError;
    }
    },
